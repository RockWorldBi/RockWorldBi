...
unsigned int nStakeMinAge = 60 * 60 * 8  * 1;	//1d, minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;	//30d, stake age of full weight
unsigned int nStakeTargetSpacing = 60;			// 60 sec block spacing

int64 nChainStartTime = 1482811858;
...
const string strMessageMagic = "ROCK WORLD BI Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    int64_t nSubsidy = 0 * COIN;
     ...
     if(nHeight == 1) nSubsidy = 24000000 * COIN;
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, const CBlockIndex* pindex)
{
     ...
     if(nPoWHeight == 1)	 nSubsidy = 24000000 * COIN;
     else         	      	 nSubsidy = 0 * COIN;
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
    ...
    if(nHeight < 1 * YEARLY_POS_BLOCK_COUNT)        nSubsidy *= 1;
    else if(nHeight < 2 * YEARLY_POS_BLOCK_COUNT)	nSubsidy *= 0.5;
    else                                            nSubsidy = 0;
...
static const int64 nTargetTimespan = 80 * 60; // 10 blocks  
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            printf("outValue:%lld valueIn:%lld \n",  GetValueOut(), nValueIn);
            const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBlock->pprev, false);
            int nProofOfStakeReward = GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, nTime, pIndex0->nHeight);
            printf("nCoinAge:%lld pindexBlock->nBits:%lld nTime:%d pindexBlock->nHeight:%d\n", nCoinAge, pindexBlock->nBits, nTime, pIndex0->nHeight);
            printf("nProofOfStakeReward:%lld GetMinFee():%lld MIN_TX_FEE:%lld\n", nProofOfStakeReward, GetMinFee(), MIN_TX_FEE);
            int64 nStandardReward = nProofOfStakeReward - GetMinFee() + MIN_TX_FEE;
            if (nStakeReward > nStandardReward)
                return DoS(100, error("ConnectInputs() : %s stake reward exceeded, block stakereward:%lld, standard stakereward:%lld", GetHash().ToString().substr(0,10).c_str(), nStakeReward, nStandardReward));
        }
        else
        {
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ROCK WORLD BI
    bool fStrictPayToScriptHash = true; // Always active in ROCK WORLD BI
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ROCK WORLD BI", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "2016.12.27 ROCK WORLD BI";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("2c7738b80713c14b592aa55707d99d840a9934f5484546d2a9c359b84a13ae6b"));
